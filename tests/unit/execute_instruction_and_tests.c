...
CU_pSuite add_and_suite_to_registry() {
    ...
    if (CU_add_test(suite, "AND Absolute,X Page Crossed", test_instr_and_absx_page) == NULL) {
        CU_cleanup_registry();
        return NULL;
    }

    if (CU_add_test(suite, "AND Absolute,Y Page Crossed", test_instr_and_absy_page) == NULL) {
        CU_cleanup_registry();
        return NULL;
    }
