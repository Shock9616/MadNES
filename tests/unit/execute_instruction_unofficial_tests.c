...
void test_instr_slo() {
    ...
    CU_ASSERT_EQUAL(processor.A, 0x12);
    CU_ASSERT_EQUAL(memory[0x2004], 0x00);
    CU_ASSERT_EQUAL(cycles, 7);
}

// ---------- Run Tests ----------

CU_pSuite add_unofficial_suite_to_registry() {
    CU_pSuite suite = CU_add_suite_with_setup_and_teardown(
        "executeInstruction Unofficial Opcode Tests", NULL, NULL, init_test, clean_test);
    ...
    if (CU_add_test(suite, "NOP ($1C)", test_instr_nop_absx) == NULL) {
        CU_cleanup_registry();
        return NULL;
    }

    if (CU_add_test(suite, "SLO", test_instr_slo) == NULL) {
        CU_cleanup_registry();
        return NULL;
    }
